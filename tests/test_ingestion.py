...
async def test_ingestion_flow_text(db: AsyncSession):
    ...
    with patch("app.services.embedding.EmbeddingService.get_embeddings", new_callable=AsyncMock) as mock_get_embeds:
        mock_get_embeds.return_value = mock_embeddings
        
        ingestion_service = IngestionService(db)
        
        # 3. Process a simple text file
        file_content = b"This is a test document content. It should be chunked and embedded."
        filename = "test.txt"
        ...
        # Check Chunks
        result = await db.execute(select(Chunk).where(Chunk.source_id == source.id))
        chunks = result.scalars().all()
        assert len(chunks) > 0
        assert chunks[0].text == "This is a test document content. It should be chunked and embedded."
        
        # Check Embeddings
        result = await db.execute(select(Embedding).where(Embedding.project_id == project.id))
