from sqlalchemy import String, ForeignKey, Integer, JSON, Boolean, Numeric, func, DateTime
...
class RetrievalMetric(Base):
    ...
    keyword_search_time_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    cache_hit_rate: Mapped[Optional[float]] = mapped_column(Numeric(5, 4), nullable=True)
    avg_vector_score: Mapped[Optional[float]] = mapped_column(Numeric(5, 4), nullable=True)
    avg_keyword_score: Mapped[Optional[float]] = mapped_column(Numeric(5, 4), nullable=True)
    avg_reranker_score: Mapped[Optional[float]] = mapped_column(Numeric(5, 4), nullable=True)
